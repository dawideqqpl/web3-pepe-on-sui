...
public class ChainSafeServerSettings : EditorWindow
{
    ...
    void Awake()
    {
        ...
        projectConfigSO = ProjectConfigScriptableObject.LoadConfig();

        if ((ProjectID == "Please Enter Your Project ID") && projectConfigSO != null && ProjectConfigScriptableObject.HasConfiguredValue(projectConfigSO.ProjectId))
        {
            ProjectID = projectConfigSO.ProjectId;
        }

        if ((ChainID == "Please Enter Your Chain ID") && projectConfigSO != null && ProjectConfigScriptableObject.HasConfiguredValue(projectConfigSO.ChainId))
        {
            ChainID = projectConfigSO.ChainId;
        }

        if ((Chain == "Please Enter Your Chain i.e Ethereum, Binance, Cronos") && projectConfigSO != null && ProjectConfigScriptableObject.HasConfiguredValue(projectConfigSO.Chain))
        {
            Chain = projectConfigSO.Chain;
        }

        if ((Network == "Please Enter Your Network i.e Mainnet, Testnet") && projectConfigSO != null && ProjectConfigScriptableObject.HasConfiguredValue(projectConfigSO.Network))
        {
            Network = projectConfigSO.Network;
        }

        if ((RPC == "Please Enter Your RPC") && projectConfigSO != null && ProjectConfigScriptableObject.HasConfiguredValue(projectConfigSO.Rpc))
        {
            RPC = projectConfigSO.Rpc;
        }
    }
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Save Settings"))
        {
            ...
            if (int.TryParse(ChainID, out int chainIdValue))
            {
                PlayerPrefs.SetInt("ChainIDInt", chainIdValue);
            }
    ...
    public void WriteNetworkFile()
    {
        ...
        StreamWriter writer2 = new StreamWriter(path1, false);
